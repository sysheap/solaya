...
use klib::util::{get_bit, get_multiple_bits, set_multiple_bits, set_or_clear_bit};
...
const _: [(); super::PAGE_SIZE] = [(); core::mem::size_of::<PageTable>()];
...
pub fn activate_page_table(satp_val: usize) {
    ...
    unsafe {
        crate::cpu::write_satp_and_fence(satp_val);
    }
}
