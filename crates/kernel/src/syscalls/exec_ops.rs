...
impl LinuxSyscallHandler {
    ...
    pub(super) fn do_execve(
        ...
        // Try VFS first: absolute paths resolve as-is, relative paths get
        // re-based against cwd.  Falls through to PROGRAMS on any failure
        // (ENOENT, EACCES, truncated read) so legacy boot paths keep working
        // until buildroot's cpio populates /bin.
        let vfs_bytes = try_read_from_vfs(filename_str, &old_cwd_str);

        let elf_arc: Arc<[u8]> = if let Some(bytes) = vfs_bytes {
