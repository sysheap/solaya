...
use crate::{
    cpu::Cpu,
    debug, fs,
    processes::{elf::ElfFile, futex, process::POWERSAVE_TID, thread::Thread},
};
...
pub fn init() {
    ...
    let init_bytes = load_init_bytes();
    let elf = ElfFile::parse(&init_bytes).expect("Cannot parse init ELF file");
    let default_env = [
    ...
    thread.lock().process().lock().set_elf_bytes(init_bytes);
