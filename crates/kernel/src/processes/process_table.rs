...
use crate::autogenerated::userspace_programs::INIT;
use crate::{
    cpu::Cpu,
    debug, fs,
...
/// Source the PID-1 ELF image.  With `rootdir_legacy` (the default), this
/// is the compile-time-embedded INIT static slice; after the feature
/// goes off, it's `/sbin/init` read from the initramfs-populated tmpfs
/// root.
#[cfg(feature = "rootdir_legacy")]
...
fn load_init_bytes() -> Arc<[u8]> {
    ...
    for path in INIT_PATHS {
        ...
        let size = node.size();
        let mut buf: alloc::vec::Vec<u8> = alloc::vec![0u8; size];
