...
build-coreutils:
    ...
    mkdir -p ./crates/kernel/compiled_userspace
    cmp -s ./target-coreutils/bin/coreutils ./crates/kernel/compiled_userspace/coreutils || cp ./target-coreutils/bin/coreutils ./crates/kernel/compiled_userspace/coreutils
    cd crates/kernel/compiled_userspace && echo "{{COREUTILS_FEATURES}}" | tr ',' '\n' | xargs -I{} ln -sf coreutils {}

build-userspace: build-coreutils
    cd userspace && cargo build --bins --artifact-dir ../crates/kernel/compiled_userspace -Z unstable-options --release
...
clean:
    ...
    rm -f crates/kernel/src/autogenerated/userspace_programs.rs
...
unit-test: build-userspace
    ...
    cargo test --release -p arch --lib --target x86_64-unknown-linux-gnu --no-default-features
...
debuguf BIN FUNC: build
    tmux new-session -d '{{debugReleaseCommand}}' \; split-window -v 'while [ ! -f .gdb-port ]; do sleep 0.1; done; {{gdb}} -ex "target remote :$(cat .gdb-port)" -ex "hbreak {{FUNC}}" -ex "c" $(pwd)/crates/kernel/compiled_userspace/{{BIN}}' \; attach
