build: build-cargo patch-symbols build-disk
...
build-coreutils:
    ...
    mkdir -p ./disk/bin
    cmp -s ./target-coreutils/bin/coreutils ./disk/bin/coreutils || cp ./target-coreutils/bin/coreutils ./disk/bin/coreutils
    cd disk/bin && echo "{{COREUTILS_FEATURES}}" | tr ',' '\n' | xargs -I{} ln -sf coreutils {}

build-userspace: build-coreutils
    cd userspace && cargo build --bins --target-dir ../target-userspace --artifact-dir ../disk/bin -Z unstable-options --release
...
clean:
    rm -rf disk/bin disk/init disk.img
