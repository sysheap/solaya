...
Kernel unit tests still reference static `INIT`/`PROG1` byte slices
for fixture ELFs; `crates/kernel/build.rs` emits
`crates/kernel/src/autogenerated/userspace_programs.rs` for that
purpose, but the generated module is now `#[cfg(test)]`-only.
